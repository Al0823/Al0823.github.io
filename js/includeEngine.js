  ...
  function include(targetId, url, callback) {
    fetch(url)
      ...
      .then(function (html) {
        ...
        // Create a container to parse HTML safely
        var temp = d.createElement("div");
        temp.innerHTML = html;

        // Extract and move non-script content
        var nodes = temp.childNodes;
        var scripts = [];

        // Collect scripts separately
        temp.querySelectorAll("script").forEach(function (s) {
          scripts.push(s);
        });

        // Remove scripts from DOM before injection
        ...
        // Execute external scripts only (CSP-safe)
